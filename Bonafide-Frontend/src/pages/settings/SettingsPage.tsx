...
import { settingsService, teacherService } from '@/services'
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (role === 'teacher') {
      teacherService.getAvailability().then((res) => {
    ...
    if (user) {
      setProfile((p) => ({
        ...
        hourlyRate: (user as any).hourlyRate ?? (user as any).hourly_rate ?? 0,
      }))
    }
  }, [user])

  const addAvailabilitySlot = () => {
    setAvailability([...availability, { day: 'Monday', slots: [{ start: '09:00', end: '17:00' }] }])
  }

  const updateAvailabilityDay = (idx: number, day: string) => {
    const updated = [...availability]
    updated[idx] = { ...updated[idx], day }
    setAvailability(updated)
  }

  const updateAvailabilityTime = (idx: number, slotIdx: number, field: 'start' | 'end', value: string) => {
    const updated = [...availability]
    const slots = [...updated[idx].slots]
    slots[slotIdx] = { ...slots[slotIdx], [field]: value }
    updated[idx] = { ...updated[idx], slots }
    setAvailability(updated)
  }
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const payload = Object.fromEntries(
        Object.entries(profile).filter(([, v]) => v !== '' && v !== null && v !== undefined)
      )
      ...
      case 'availability':
        return (
          <motion.div
            ...
            className="space-y-4"
          >
            <p className="text-sm text-gray-500">Set your weekly availability for booking sessions</p>
              ...
              <div key={idx} className="flex flex-wrap items-end gap-3 rounded-xl border p-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onChange={(e) => updateAvailabilityTime(idx, 0, 'start', e.target.value)}
                ...
                <div>
                  ...
                  <Select
                    ...
                    onChange={(e) => updateAvailabilityTime(idx, 0, 'end', e.target.value)}
                    options={TIME_SLOTS.map((t) => ({ value: t, label: t }))}
                  />
                </div>
                <Button variant="ghost" size="icon" onClick={() => removeAvailabilitySlot(idx)}>
                  <X className="h-4 w-4" />
                </Button>
              </div>
            ))}
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={addAvailabilitySlot} className="gap-2">
