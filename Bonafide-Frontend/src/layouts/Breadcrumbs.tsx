...
export function Breadcrumbs() {
  ...
  return (
    <nav className="flex items-center gap-1.5 text-sm text-gray-500 mb-5" aria-label="Breadcrumb">
      ...
      {crumbs.map((crumb) => {
        const isRolePrefix = crumb.href === '/parent' || crumb.href === '/teacher' || crumb.href === '/admin'
        return (
        <span key={crumb.href} className="flex items-center gap-1.5">
          <ChevronRight className="h-3 w-3 text-gray-400" aria-hidden="true" />
          {crumb.isLast || isRolePrefix ? (
            <span className={crumb.isLast ? 'text-gray-900 font-medium' : 'text-gray-500'} aria-current={crumb.isLast ? 'page' : undefined}>{crumb.label}</span>
          ) : (
