...
export default function TeacherDashboard() {
  ...
  const { stats, todaySessions, recentStudents, upcomingWeek, recentEarnings } = data
  ...
  const statCards = [
    ...
    { label: 'Average Rating', value: stats.averageRating, icon: Star, color: 'text-orange-600 bg-orange-100', suffix: '/5' },
    { label: 'Monthly Revenue', value: `$${stats.monthlyRevenue}`, icon: DollarSign, color: 'text-emerald-600 bg-emerald-100' },
  ]
                  ...
                  <div key={earning.id} className="flex items-center justify-between px-5 py-3.5 text-sm">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-8 w-8">
                        <AvatarImage src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${earning.student.toLowerCase().replace(' ', '-')}`} />
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">{earning.date}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold text-green-600">${earning.amount}</p>
                      <Badge variant="success" size="sm">{earning.status}</Badge>
                    </div>
                  </div>
                ))}
              </div>
