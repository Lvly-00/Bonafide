...
export default function MatchingPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (priceMin) {
      result = result.filter((t) => t.hourlyRate >= parseInt(priceMin))
    }
    if (priceMax) {
      result = result.filter((t) => t.hourlyRate <= parseInt(priceMax))
    }
    ...
    return result
  }, [teachers, searchQuery, selectedSubjects, priceMin, priceMax, ratingFilter, locationFilter, sortBy])
  ...
  const hasActiveFilters = selectedSubjects.length > 0 || priceMin || priceMax || ratingFilter > 0 || locationFilter
              ...
              <CardContent className="p-5">
                <div className="flex flex-wrap items-start gap-6">
                  ...
                  <div className="flex items-center gap-4">
                    <div>
                      <p className="mb-1.5 text-xs font-semibold text-gray-500">Price Range</p>
                      <div className="flex items-center gap-2">
                        <Input
                          type="number"
                          placeholder="Min"
                ...
                <CardContent className={`flex-1 p-5 ${viewMode === 'list' ? 'flex items-center gap-5' : ''}`}>
                  <div className={`flex ${viewMode === 'list' ? 'items-center gap-4 flex-1' : 'flex-col items-center text-center'}`}>
                    ...
                    <div className={`${viewMode === 'list' ? 'flex-1 text-left' : ''}`}>
                      ...
                      <div className={`mt-2 flex items-center gap-2 text-xs text-gray-500 ${viewMode === 'list' ? '' : 'justify-center'}`}>
                    ...
                    <div className={`${viewMode === 'list' ? 'flex items-center gap-4 text-right' : 'mt-4 flex w-full items-center justify-between border-t border-border pt-4'}`}>
                      <div className={viewMode === 'list' ? '' : ''}>
                        <p className="text-lg font-bold text-primary">${teacher.hourlyRate}</p>
                        <p className="text-xs text-gray-400">per hour</p>
                      </div>
                      <div className={`flex gap-2 ${viewMode === 'list' ? '' : ''}`}>
