...
export default function AddChildWizard() {
  ...
  const startMatching = useCallback(async () => {
    ...
    const match = findBestMatch(teachers, form)
    setMatchedTeacher(match)
    setMatching(false)
    setDirection(1)
    setStep(6)
  }, [teachers, form])
  ...
  const otherMatches = teachers
    .filter((t) => t.id !== matchedTeacher?.id)
  ...
  const matchResult = matchedTeacher ? (
    <div className="space-y-6">
      <motion.div
      ...
      >
        ...
        <h2 className="text-xl font-bold">Profile Created & Match Found!</h2>
        <p className="mt-1 text-sm text-gray-500">
          Based on {form.name}'s profile, we found the perfect teacher
        </p>
      </motion.div>
      ...
      >
        <Card className="relative overflow-hidden border-2 border-primary/20">
          <div className="absolute right-0 top-0 rounded-bl-xl bg-primary px-4 py-1 text-[11px] font-medium text-white">
            Best Match
          </div>
          <CardContent className="p-6">
            <div className="flex flex-col items-center gap-4 sm:flex-row">
              <Avatar className="h-20 w-20">
                <AvatarImage src={matchedTeacher!.avatar} />
                <AvatarFallback className="text-lg">
                  {matchedTeacher!.name.split(' ').map((n) => n[0]).join('')}
                </AvatarFallback>
              </Avatar>
              <div className="flex-1 text-center sm:text-left">
                <h3 className="text-lg font-bold">{matchedTeacher!.name}</h3>
                <div className="mt-1 flex items-center justify-center gap-1 sm:justify-start">
                  <div className="flex">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className={`h-4 w-4 ${
                          i < Math.floor(matchedTeacher!.rating)
                  ...
                  <span className="text-sm text-gray-500">({matchedTeacher!.rating})</span>
                </div>
                <div className="mt-1 flex items-center justify-center gap-2 text-sm text-gray-500 sm:justify-start">
                  <MapPin className="h-3.5 w-3.5" />
                  {matchedTeacher!.location}
                  <span className="text-gray-300">&middot;</span>
                  <span>{matchedTeacher!.experience} yrs exp</span>
                </div>
                <div className="mt-2 flex flex-wrap justify-center gap-1.5 sm:justify-start">
                  {matchedTeacher!.subjects.map((subj) => (
              ...
              <div className="text-center sm:text-right">
                <p className="text-2xl font-bold text-primary">${matchedTeacher!.hourlyRate}</p>
                <p className="text-xs text-gray-400">per hour</p>
              </div>
            </div>

            <p className="mt-4 text-sm leading-relaxed text-gray-600">{matchedTeacher!.bio}</p>

            <div className="mt-5 flex flex-wrap gap-3">
              <Button
                className="flex-1 gap-2"
                  onClick={() =>
                    navigate(`/parent/booking/${matchedTeacher!.id}`, {
                      state: { preselectedChild: createdChild },
                    })
                  }
              >
                <BookOpen className="h-4 w-4" /> Book a Session
              </Button>
              <Button
                variant="outline"
                className="flex-1 gap-2"
                onClick={() => navigate(`/parent/teacher/${matchedTeacher!.id}`)}
              >
              ...
              <Button
                ...
                size="icon"
                className="shrink-0"
                onClick={() => toggleFavorite(matchedTeacher!.id)}
              >
                <Heart
                  className={`h-5 w-5 ${
                    favorites.has(matchedTeacher!.id) ? 'fill-red-500 text-red-500' : 'text-gray-400'
                  }`}
                />
              </Button>
            </div>
              ...
              <CardContent className="flex items-center gap-4 p-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-gray-500">
                    {teacher.subjects.slice(0, 3).join(', ')} &middot; ${teacher.hourlyRate}/hr
                  </p>
                </div>
                <div className="flex gap-2">
                  <Button
                  ...
                  <Button
                    size="sm"
                    onClick={() => navigate(`/parent/booking/${teacher.id}`, { state: { preselectedChild: createdChild } })}
                  >
                    Book
                  </Button>
                </div>
      ...
      <div className="flex items-center justify-between pt-2">
        <Button variant="outline" onClick={() => navigate(ROUTES.CHILD_PROFILE)}>
          <ChevronLeft className="mr-1 h-4 w-4" /> Back to Children
        </Button>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => navigate(ROUTES.MATCHING)}>
            <RefreshCw className="mr-1.5 h-4 w-4" /> Browse All Teachers
          </Button>
          <Button onClick={() => navigate(ROUTES.PARENT_DASHBOARD)}>
  ...
  return (
    ...
    >
      ...
      {step === 6 && matchedTeacher && matchResult}
    </motion.div>
  )
}
